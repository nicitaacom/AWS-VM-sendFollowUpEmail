...
// DO NOT use this function in VM - for some reason it work with smth else but doesn't work with redis
// I tried to change environment from node 22 to node 20 and ask chatGPT - useless
async function decryptTelegramChatId(
  encryptedTelegramChatId: string,
  redisKeyFor: string
...
// DO NOT use this function in VM - for some reason it work with smth else but doesn't work with redis
async function decryptTelegramBotToken(
...
async function decryptDiscordWebhookUrl(
  encryptedDiscordWebhookUrl: string,
  redisKeyFor: string,
): Promise<[string] | string> {
  if (typeof window === "undefined") {
    try {
